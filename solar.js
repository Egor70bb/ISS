...
async function findTransits(satrec, observer){
  ...
  const searchReach=SUN_RADIUS_DEG+(observer.radius/360)*57.2958+0.12;
  for(let i=0;i<passWindows.length;i++){
    ...
    if(i%8===0){updateProgress(74+Math.round(10*i/Math.max(1,passWindows.length)),'Verifica ravvicinata dei passaggi diurni…');await breathe();}
  }
  ...
  for(let i=0;i<candidates.length;i++){
    ...
    if(best.sep<=SUN_RADIUS_DEG) events.push(best);
    await breathe();
  }
...
function geometry(satrec,date,lat,lon,heightM){
  ...
  const rangeKm=Number.isFinite(look.rangeSat)?look.rangeSat:null;
  const issArcsec=rangeKm?2*Math.atan(ISS_LENGTH_KM/(2*rangeKm))*180/Math.PI*3600:null;
  return {time:date.getTime(),sep,issAlt,issAz,sunAlt:hor.altitude,sunAz:hor.azimuth,ra:eq.ra,dec:eq.dec,rangeKm,issArcsec};
}
...
function optimizeLocation(satrec,observer,candidate){
  ...
  const halfChord=Math.sqrt(Math.max(0,SUN_RADIUS_DEG**2-best.sep**2));
...
function renderResults(events,observer,epoch){
  ...
  if(!events.length){
    emptyState.classList.remove('hidden');
    emptyState.innerHTML='<div class="sun-symbol">☀</div><h3>Nessun transito solare trovato</h3><p>Non risultano attraversamenti del disco entro il raggio e il periodo selezionati. Riprova nei prossimi giorni: i dati orbitali vengono aggiornati.</p>';
    return;
  }
...
function eventCard(e,index){
  ...
  const quality=e.sunAlt>=30&&e.sep<=0.12?'Ottima':e.sunAlt>=15?'Buona':'Difficile';
  return `<article class="event"><div class="event-head"><div><h3>Transito solare #${index+1}</h3><div class="event-time">${capitalize(time)}</div></div><span class="badge ${confirm?'confirm':'plan'}">${confirm?'DA CONFERMARE 24 H PRIMA':'PRELIMINARE'}</span></div><div class="metrics"><div class="metric"><span>Distanza</span><strong>${e.offsetKm.toFixed(2)} km</strong></div><div class="metric"><span>Altezza Sole</span><strong>${e.sunAlt.toFixed(1)}°</strong></div><div class="metric"><span>Azimut</span><strong>${e.sunAz.toFixed(1)}°</strong></div><div class="metric"><span>Durata stimata</span><strong>${e.durationEstimate.toFixed(2)} s</strong></div><div class="metric"><span>Dimensione ISS</span><strong>${e.issArcsec?e.issArcsec.toFixed(1)+'″':'—'}</strong></div><div class="metric"><span>Distanza ISS</span><strong>${e.rangeKm?e.rangeKm.toFixed(0)+' km':'—'}</strong></div><div class="metric"><span>Coordinate migliori</span><strong>${e.lat.toFixed(5)}, ${e.lon.toFixed(5)}</strong></div><div class="metric"><span>Distanza dal centro</span><strong>${e.sep.toFixed(3)}°</strong></div><div class="metric"><span>Qualità</span><strong>${quality}</strong></div></div><div class="event-actions"><a href="${maps}" target="_blank" rel="noopener">Apri punto su Google Maps</a><a href="${calendar}" download="transito-iss-sole-${index+1}.ics">Aggiungi al calendario</a></div>${dwarfEventCard(e,quality)}</article>`;
}

function dwarfEventCard(e,quality){
  const exposure=e.sunAlt>=30?'1/1000 s':'1/500 s';
  return `<details class="dwarf-event solar-event"><summary>Scheda di ripresa DWARF 3 per questo evento</summary><div class="capture-content"><div class="capture-grid"><div><span>Video</span><strong>1080p · 60 fps</strong></div><div><span>Esposizione iniziale</span><strong>${exposure}</strong></div><div><span>Gain</span><strong>0</strong></div><div><span>Filtro</span><strong>Filtro solare</strong></div><div><span>Avvio video</span><strong>−45 secondi</strong></div><div><span>Fine video</span><strong>+20 secondi</strong></div><div><span>Sun Track</span><strong>Attivo</strong></div><div><span>Condizione stimata</span><strong>${quality}</strong></div></div><ol class="capture-steps"><li>Monta e verifica il filtro solare prima di puntare il Sole.</li><li>Centra l’intero disco solare nel teleobiettivo e controlla il fuoco.</li><li>Attiva <strong>Sun Track</strong>, poi passa a <strong>Video 1080p/60 fps</strong>.</li><li>Imposta Gain 0 e prova l’esposizione proposta, evitando qualsiasi area sovraesposta.</li><li>Avvia la registrazione 45 secondi prima dell’ora prevista e non toccare più il telescopio.</li></ol><div class="capture-warning">Conferma sempre orario e coordinate il giorno stesso. Non rimuovere mai il filtro durante il puntamento solare.</div></div></details>`;
}
...
function icsLink(e,index){
  ...
  const body=['BEGIN:VCALENDAR','VERSION:2.0','BEGIN:VEVENT',`UID:iss-solar-${index}-${e.time}@transit-finder`,`DTSTAMP:${fmt(new Date())}`,`DTSTART:${fmt(start)}`,`DTEND:${fmt(end)}`,'SUMMARY:Transito ISS davanti al Sole',`DESCRIPTION:Coordinate consigliate ${e.lat.toFixed(5)}, ${e.lon.toFixed(5)}. Confermare previsione il giorno stesso.`,'END:VEVENT','END:VCALENDAR'].join('\r\n');
...
function updateProgress(p,text){progressWrap.classList.remove('hidden');progressBar.style.width=`${p}%`;progressPercent.textContent=`${p}%`;progressText.textContent=text;}
function setRunning(running){$('searchButton').disabled=running;$('stopButton').classList.toggle('hidden',!running);if(!running)setTimeout(()=>progressWrap.classList.add('hidden'),900);}
