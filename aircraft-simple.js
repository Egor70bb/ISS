...
let flightData=null;
for(const id of ['calculateButton','airport','days','bodyFilter','movementFilter']){
...
function directionsUrl(code,c){
  const o=ORIGINS[code];
  return `https://www.google.com/maps/dir/?api=1&origin=${o.lat.toFixed(6)},${o.lon.toFixed(6)}&destination=${c.spot.lat.toFixed(6)},${c.spot.lon.toFixed(6)}&travelmode=driving`;
}
function updateFeedAge(code){
  const bucket=flightData?.airports?.[code]; if(!bucket) return;
  const iso=bucket.updated_at||flightData.generated_at;
  const d=new Date(iso); if(!Number.isFinite(d.getTime())) return;
  const age=Math.max(0,Math.round((Date.now()-d.getTime())/60000));
  $('dataAge').textContent=`Feed ${AIRPORTS[code].name}: ${d.toLocaleDateString('it-IT',{day:'2-digit',month:'2-digit'})}, ${d.toLocaleTimeString('it-IT',{hour:'2-digit',minute:'2-digit'})} · ${age} min fa`;
}
...
function calculate(){
  ...
  for(const f of flights){
    const key=`${f.movement}|${f.scheduled_iso}|${f.flight||''}`;
  ...
  render(candidates,flights.length,bucket,days,code);
}
function render(candidates,flightCount,bucket,days,code){
  $('summary').classList.remove('hidden');
  $('summary').innerHTML=`<span><strong>${flightCount}</strong> movimenti analizzati</span><span><strong>${candidates.length}</strong> candidati mostrati</span><span>Orizzonte: <strong>${days} gg</strong></span><span>Origine distanza: <strong>${escapeHtml(ORIGINS[code].label)}</strong></span><span>Feed: <strong>${escapeHtml(bucket.status||'—')}</strong></span>`;
  if(!candidates.length){
    ...
    $('results').innerHTML=`<strong>Nessun evento candidato.</strong><br>${extra}`;
    return;
  }
  let lastDay='';
...
async function load(){
  try{
    ...
    const r=await fetch(`${DATA_URL}?v=${Date.now()}`,{cache:'no-store',signal:controller.signal});
    clearTimeout(timer);
    ...
    $('results').textContent=`Impossibile caricare gli orari voli: ${e.name==='AbortError'?'timeout 5 s':e.message}`;
  }
}
load();
